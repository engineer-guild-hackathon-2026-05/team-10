...
**使用例**: `groove`（リズムに乗る） / `chill-listener` / `neutral` / `bass-driven`
...
**本プロジェクトでの用途**: `ai-recognition/` で学習したモデルを変換し、端末上で3状態スコアを推論。
...
### ReactionSpan
**定義**: 検出された身体・生理反応区間（開始・終了秒 + 3状態スコア）を表す Swift 型。
...
**本プロジェクトでの用途**: `ai-recognition/` でモーション+心拍特徴量から3状態スコアを学習。学習後 Core ML へ変換。
