...
| 選択肢 | メリット | デメリット |
|---|---|---|
| **ネイティブ iOS (SwiftUI)** | AirPods 頭部モーション・MusicKit に直接アクセス可能 | 開発コストが高い |
...
プロダクトの差別化軸である「身体的モメンタムの可視化」は、AirPods 頭部モーションと MusicKit 再生位置へのネイティブアクセスなしには実現不可能であると判断した。
...
HealthKit / 心拍連携は ADR-0006 により MVP から削除済み。ネイティブ iOS 採用の根拠は、現在は AirPods 頭部モーションと MusicKit に集約される。
