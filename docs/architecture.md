...
| Core ML | 端末上の反応スコア補助推論 | `ai-recognition/` の3状態（groove / chill / neutral）モデル候補と特徴量から、iOS側の `ReactionScore` を算出（DECISION-02） |
...
| 技術 | 用途 | 選定理由 |
|------|------|----------|
| TensorFlow.js / Create ML | モーションからの3状態ラベル学習 | `ai-recognition/` のMVPは groove / chill / neutral で収集し、iOSも同じ3状態で表示・保存 |
...
- **ReactionClassifier / ReactionScoringService**: Core ML 推論候補と特徴量から groove / chill / neutral の3状態スコアを算出
